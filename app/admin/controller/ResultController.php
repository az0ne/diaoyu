...
class ResultController extends DefaultController {
    ...
    public function view() {
        ...
        //查询操作
        $sql = "SELECT * FROM `".$resultDB->getTableName()."` WHERE {$whereStr} ORDER BY {$orderStr}";
        $data = $resultDB->getData($sql, $whereArr, 20);
        
        $pageStr = $resultDB->getPageStr();
        //读取外键数据
    ...
    public function get() {
        ...
        $sql = "SELECT * FROM `".$resultDB->getTableName()."` WHERE {$whereStr}";
        $row = $resultDB->getRow($sql, $whereArr);
        $row['create_time'] = date('Y-m-d', $row['create_time']);
        
        $row ? Message::showSucc('获取单条结语数据', $row) : Message::showError('没有获取到指定数据，可能已被删除', $row);
    }
    ...
    public function create() {
        ...
        $rs = $resultDB->insert($insertArr);
        Log::write($this->adminUserName, $this->ip, $resultDB->insertId(), self::$controller . "__" . self::$action, $rs);
        $rs ? Message::showSucc('添加成功') : Message::showError('添加失败：' . implode(" ", $resultDB->getErrorInfo()));
    }
    ...
    public function update() {
        ...
        $sql = "SELECT * FROM `".$resultDB->getTableName()."` WHERE `id` = ?";
        $row = $resultDB->getRow($sql, array_values($whereArr));
        ...
        $rs = $resultDB->update($changeNewRow, $whereArr);
        //获取被修改数据写入日志
        ...
        //更新提示
        $rs ? Message::showSucc('更新成功', $changeNewRow) : Message::showError('更新失败：' . implode(" ", $resultDB->getErrorInfo()));
    }
    ...
    public function delete() {
        ...
        $rs = $resultDB->delete($whereArr);
        Log::write($this->adminUserName, $this->ip, $_GET['id'], self::$controller . "__" . self::$action, $rs, Log::arrayToLog($row));
        $rs ? Message::showSucc('删除成功') : Message::showError('删除失败：' . implode(" ", $resultDB->getErrorInfo()));
    }
    ...
    public function logicDelete() {
        ...
        $rs = $resultDB->update($updateArr, $whereArr);
    ...
    public function logicResume() {
        ...
        $rs = $resultDB->update($updateArr, $whereArr);
        Log::write($this->adminUserName, $this->ip, $_GET['id'], self::$controller . "__" . self::$action, $rs);
        $rs ? Message::showSucc('恢复成功') : Message::showError('恢复失败：' . implode(" ", $resultDB->getErrorInfo()));
    }
}
?>
