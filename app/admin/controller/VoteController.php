...
class VoteController extends DefaultController {
    ...
    public function view() {
        $voteDB = new VoteModelDB();
        //普通条件，检查GET参数
        $fieldArr = $voteDB->getFields();
        ...
        //查询操作
        $sql = "SELECT * FROM `".$voteDB->getTableName()."` WHERE {$whereStr} ORDER BY {$orderStr}";
        $data = $voteDB->getData($sql, $whereArr, 20);
        
        $pageStr = $voteDB->getPageStr();
        //读取外键数据
        $kind = DictConfig::$voteKind;
        $this->setView('kind', $kind);
         
        //模版显示
        $this->setView('pageStr', $pageStr);
        $this->setView('data', $data);
        $this->display('Vote.html');
    }
    ...
    public function get() {
        ...
        $sql = "SELECT * FROM `".$voteDB->getTableName()."` WHERE {$whereStr}";
        $row = $voteDB->getRow($sql, $whereArr);
        $row['create_time'] = date('Y-m-d', $row['create_time']);
        
        $row ? Message::showSucc('获取单条测试数据', $row) : Message::showError('没有获取到指定数据，可能已被删除', $row);
    }
    ...
    public function create() {
        ...
        $rs = $voteDB->insert($insertArr);
        Log::write($this->adminUserName, $this->ip, $voteDB->insertId(), self::$controller . "__" . self::$action, $rs);
        $rs ? Message::showSucc('添加成功') : Message::showError('添加失败：' . implode(" ", $voteDB->getErrorInfo()));
    }
    ...
    public function update() {
        ...
        $sql = "SELECT * FROM `".$voteDB->getTableName()."` WHERE `id` = ?";
        $row = $voteDB->getRow($sql, array_values($whereArr));
        ...
        $rs = $voteDB->update($changeNewRow, $whereArr);
        //获取被修改数据写入日志
        ...
        //更新提示
        $rs ? Message::showSucc('更新成功', $changeNewRow) : Message::showError('更新失败：' . implode(" ", $voteDB->getErrorInfo()));
    }
    ...
    public function delete() {
        ...
        $rs = $voteDB->delete($whereArr);
        Log::write($this->adminUserName, $this->ip, $_GET['id'], self::$controller . "__" . self::$action, $rs, Log::arrayToLog($row));
        $rs ? Message::showSucc('删除成功') : Message::showError('删除失败：' . implode(" ", $voteDB->getErrorInfo()));
    }
    ...
    public function logicDelete() {
        ...
        $rs = $voteDB->update($updateArr, $whereArr);
    ...
    public function logicResume() {
        ...
        $rs = $voteDB->update($updateArr, $whereArr);
        Log::write($this->adminUserName, $this->ip, $_GET['id'], self::$controller . "__" . self::$action, $rs);
        $rs ? Message::showSucc('恢复成功') : Message::showError('恢复失败：' . implode(" ", $voteDB->getErrorInfo()));
    }
}
?>
