...
class MatchController extends DefaultController {
    ...
    public function view() {
        $matchDB = new MatchModelDB();
        //普通条件，检查GET参数
        $fieldArr = $matchDB->getFields();
        ...
        //查询操作
        $sql = "SELECT * FROM `".$matchDB->getTableName()."` WHERE {$whereStr} ORDER BY {$orderStr}";
        $data = $matchDB->getData($sql, $whereArr, 20);
        
        $pageStr = $matchDB->getPageStr();
        //读取外键数据
        
        //模版显示
        $this->setView('pageStr', $pageStr);
        $this->setView('data', $data);
        $this->display('Match.html');
    }
    ...
    public function get() {
        ...
        $sql = "SELECT * FROM `".$matchDB->getTableName()."` WHERE {$whereStr}";
        $row = $matchDB->getRow($sql, $whereArr);
        $row['create_time'] = date('Y-m-d', $row['create_time']);
        
        $row ? Message::showSucc('获取单条配对数据', $row) : Message::showError('没有获取到指定数据，可能已被删除', $row);
    }
    ...
    public function create() {
        ...
        $rs = $matchDB->insert($insertArr);
        Log::write($this->adminUserName, $this->ip, $matchDB->insertId(), self::$controller . "__" . self::$action, $rs);
        $rs ? Message::showSucc('添加成功') : Message::showError('添加失败：' . implode(" ", $matchDB->getErrorInfo()));
    }
    ...
    public function update() {
        ...
        $sql = "SELECT * FROM `".$matchDB->getTableName()."` WHERE `id` = ?";
        $row = $matchDB->getRow($sql, array_values($whereArr));
        ...
        $rs = $matchDB->update($changeNewRow, $whereArr);
        //获取被修改数据写入日志
        ...
        //更新提示
        $rs ? Message::showSucc('更新成功', $changeNewRow) : Message::showError('更新失败：' . implode(" ", $matchDB->getErrorInfo()));
    }
    ...
    public function delete() {
        ...
        $rs = $matchDB->delete($whereArr);
        Log::write($this->adminUserName, $this->ip, $_GET['id'], self::$controller . "__" . self::$action, $rs, Log::arrayToLog($row));
        $rs ? Message::showSucc('删除成功') : Message::showError('删除失败：' . implode(" ", $matchDB->getErrorInfo()));
    }
    ...
    public function logicDelete() {
        ...
        $rs = $matchDB->update($updateArr, $whereArr);
    ...
    public function logicResume() {
        ...
        $rs = $matchDB->update($updateArr, $whereArr);
        Log::write($this->adminUserName, $this->ip, $_GET['id'], self::$controller . "__" . self::$action, $rs);
        $rs ? Message::showSucc('恢复成功') : Message::showError('恢复失败：' . implode(" ", $matchDB->getErrorInfo()));
    }
}
?>
