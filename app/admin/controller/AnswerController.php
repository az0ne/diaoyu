...
class AnswerController extends DefaultController {
    ...
    public function get() {
        ...
        $sql = "SELECT * FROM `".$answerDB->getTableName()."` WHERE {$whereStr}";
        $row = $answerDB->getRow($sql, $whereArr);
        $row['create_time'] = date('Y-m-d', $row['create_time']);
        
        $row ? Message::showSucc('获取单条选项数据', $row) : Message::showError('没有获取到指定数据，可能已被删除', $row);
    }
    ...
    public function create() {
        ...
        $rs = $answerDB->insert($insertArr);
        Log::write($this->adminUserName, $this->ip, $answerDB->insertId(), self::$controller . "__" . self::$action, $rs);
        $rs ? Message::showSucc('添加成功') : Message::showError('添加失败：' . implode(" ", $answerDB->getErrorInfo()));
    }
    ...
    public function update() {
        ...
        $sql = "SELECT * FROM `".$answerDB->getTableName()."` WHERE `id` = ?";
        $row = $answerDB->getRow($sql, array_values($whereArr));
        ...
        $rs = $answerDB->update($changeNewRow, $whereArr);
        //获取被修改数据写入日志
        ...
        //更新提示
        $rs ? Message::showSucc('更新成功', $changeNewRow) : Message::showError('更新失败：' . implode(" ", $answerDB->getErrorInfo()));
    }
    ...
    public function delete() {
        ...
        $rs = $answerDB->delete($whereArr);
        Log::write($this->adminUserName, $this->ip, $_GET['id'], self::$controller . "__" . self::$action, $rs, Log::arrayToLog($row));
        $rs ? Message::showSucc('删除成功') : Message::showError('删除失败：' . implode(" ", $answerDB->getErrorInfo()));
    }
    ...
    public function logicDelete() {
        ...
        $rs = $answerDB->update($updateArr, $whereArr);
    ...
    public function logicResume() {
        ...
        $rs = $answerDB->update($updateArr, $whereArr);
        Log::write($this->adminUserName, $this->ip, $_GET['id'], self::$controller . "__" . self::$action, $rs);
        $rs ? Message::showSucc('恢复成功') : Message::showError('恢复失败：' . implode(" ", $answerDB->getErrorInfo()));
    }
}
?>
