...
include 'global.php';
    ...
    if (!is_dir(PATH_DATA)) {
        $str .= '<div class="text_red">数据目录检测失败，当前设置：'.PATH_DATA.'不为目录</div>';
    } else {
        $str .= '<div class="text_green">数据目录检测成功，当前设置：'.PATH_DATA.'...</div>';
    }
    if (!is_dir(PATH_CACHE)) {
        $str .= '<div class="text_red">缓存数据目录检测失败，当前设置：'.PATH_CACHE.'不为目录</div>';
    } else {
        $str .= '<div class="text_green">缓存数据目录检测成功，当前设置：'.PATH_CACHE.'...</div>';
    }
    if (!is_dir(PATH_APPLOG)) {
        $str .= '<div class="text_red">日志目录检测失败，当前设置：'.PATH_APPLOG.'不为目录</div>';
    } else {
        $str .= '<div class="text_green">日志目录检测成功，当前设置：'.PATH_APPLOG.'...</div>';
    }

    try {
        $dsn = "mysql:dbname=".DBConfig::$config['mysql']['default']['master']['database'].";port=".DBConfig::$config['mysql']['default']['master']['port'].";host=".DBConfig::$config['mysql']['default']['master']['host'].";user=".DBConfig::$config['mysql']['default']['master']['user'].";pass=".DBConfig::$config['mysql']['default']['master']['pass'];
        $db = @mysqli_connect( DBConfig::$config['mysql']['default']['master']['host'] , DBConfig::$config['mysql']['default']['master']['user'] , DBConfig::$config['mysql']['default']['master']['pass'] , DBConfig::$config['mysql']['default']['master']['name'] , DBConfig::$config['mysql']['default']['master']['port']);
    } catch (Exception $e) {}
    if ($db) {
    ...
    if (is_dir(PATH_DATA) && @file_put_contents(PATH_DATA . '_test.test', '') !== false) {
        $str .= '<div class="text_green">数据文件目录可用性检测成功，当前数据目录设置为：'.PATH_DATA.'...</div>';
        unlink(PATH_DATA . '_test.test');
    } else {
        $str .= '<div class="text_red">数据文件目录可用性检测失败，当前数据目录设置为：“'.PATH_DATA.'”，如目录设置正确，请检查目录权限</div>';
    }
    if (is_dir(PATH_CACHE) && @file_put_contents(PATH_CACHE . '_test.test', '') !== false) {
        $str .= '<div class="text_green">cache文件目录可用性检测成功，当前缓存数据目录设置为：'.PATH_CACHE.'...</div>';
        unlink(PATH_CACHE . '_test.test');
    } else {
        $str .= '<div class="text_red">cache文件目录可用性检测失败，当前缓存数据目录设置为：“'.PATH_CACHE.'”，如目录设置正确，请检查目录权限</div>';
    }
    if (is_dir(PATH_APPLOG) && @file_put_contents(PATH_APPLOG . '._test.test', '') !== false) {
        $str .= '<div class="text_green">日志文件目录可用性检测成功，当前LOG目录设置为：'.PATH_APPLOG.'...</div>';
        unlink(PATH_APPLOG . '._test.test');
    } else {
        $str .= '<div class="text_red">日志文件目录可用性检测失败，当前LOG目录设置为：“'.PATH_APPLOG.'”，如目录设置正确，请检查目录权限，不记录日志无影响</div>';
    }
